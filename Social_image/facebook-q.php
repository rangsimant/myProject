...
class GetFacebook
{
	...
	function __construct()
	{
		...
		$this->db = new Db();
		$this->facebook_id = $this->db->query("SELECT account_id_user,account_username ,account_last_datetime FROM account WHERE account_channel ='facebook' ORDER BY account_timestamp ASC");
	}

	public function Download_Picture($q_facebook){
		date_default_timezone_set("Asia/Bangkok");
		$groupPost="";
		$id_user = json_decode($q_facebook);
		...
		//foreach ($this->facebook_id as $key => $id_user) {
			...
			echo "\n".$this->username;
			$this->db->query("UPDATE account SET account_timestamp = :account_timestamp WHERE account_username = :username AND account_channel ='facebook'",
			...
			try {
				$this->getPost($url);
				$this->db->query("UPDATE account SET account_last_datetime = :last_datetime WHERE account_username = :username AND account_channel ='facebook'",
