...
class GetInstagram
{
	...
	function __construct()
	{
	    ...
	    $this->instagram_id = $this->db->query("SELECT account_id_user,account_username ,account_last_datetime FROM account WHERE account_channel ='instagram' AND account_available = 'open'");
	}
	...
	public function Download_Picture(){
		...
		foreach ($this->instagram_id as $key => $id_user) {
			...
			echo "\n".$this->username;
			$this->db->query("UPDATE account SET account_timestamp = :account_timestamp WHERE account_username = :username AND account_available = 'open'",
			...
			$this->db->query("UPDATE account SET account_last_datetime = :last_datetime WHERE account_username = :username AND account_available = 'open'",
